...
<head>
    ...
    <title>${empty ct ? 'Thêm mới' : 'Chỉnh sửa'} Lịch trình - Vé Tàu VN</title>
...
<body>
    ...
    <main class="main-content">
        <div class="form-card">
            <h2 class="form-title">${empty ct ? '➕ Thêm mới Chuyến Tàu' : '✏️ Chỉnh sửa Lịch trình'}</h2>
            ...
            <form action="${pageContext.request.contextPath}/admin/chuyen-tau" method="post">
                <input type="hidden" name="id" value="${ct.id}">
                
                <div class="form-grid">
                    <div class="form-group">
                        <label>Mã Chuyến (VD: SE1-20260507)</label>
                        <input type="text" name="maChuyen" value="${ct.maChuyen}" class="form-control" required>
                    </div>
                    
                    <div class="form-group">
                        <label>Chọn Đoàn Tàu</label>
                        <select name="tauId" class="form-control" required>
                            <option value="">-- Chọn Tàu vận hành --</option>
                            <c:forEach var="tau" items="${listTau}">
                                <option value="${tau.id}" ${ct.tauId == tau.id ? 'selected' : ''}>${tau.maTau} - ${tau.tenTau}</option>
                            </c:forEach>
                        </select>
                    </div>

                    <div class="form-group full-width">
                        <label>Ngày Chạy (Ngày khởi hành)</label>
                        <input type="date" name="ngayChay" value="${ct.ngayChay}" class="form-control" required>
                    </div>

                    <div class="form-group">
                        <label>Giờ Khởi Hành</label>
                        <input type="datetime-local" name="gioKhoiHanh" value="${ct.gioKhoiHanh}" class="form-control" required>
                    </div>

                    <div class="form-group">
                        <label>Giờ Đến (Dự kiến - Tùy chọn)</label>
                        <input type="datetime-local" name="gioDenDuKien" value="${ct.gioDenDuKien}" class="form-control">
                    </div>

                    <div class="form-group full-width">
                        <label>Trạng thái</label>
                        <select name="trangThai" class="form-control">
                            <option value="Hoạt động" ${ct.trangThai == 'Hoạt động' ? 'selected' : ''}>🟢 Sắp khởi hành / Đang chạy</option>
                            <option value="Hoàn thành" ${ct.trangThai == 'Hoàn thành' ? 'selected' : ''}>⚪ Đã hoàn thành lộ trình</option>
                            <option value="Hủy" ${ct.trangThai == 'Hủy' ? 'selected' : ''}>🔴 Hủy chuyến</option>
                        </select>
                    </div>
                </div>
                
                <div class="btn-group">
                    <a href="${pageContext.request.contextPath}/admin/chuyen-tau" class="btn btn-cancel">Quay lại</a>
                    <button type="submit" class="btn btn-save">💾 Lưu Lịch trình</button>
                </div>
            </form>
        </div>
    </main>
